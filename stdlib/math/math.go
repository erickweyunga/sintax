...
func Load() *types.Module {
	return &types.Module{
		...
		Info: []types.FuncInfo{
			...
			{"random_range(a, b)", "Random number between a and b"},
			{"sign(n)", "Sign: -1, 0, or 1"},
			{"clamp(n, min, max)", "Clamp number between min and max"},
			{"sum(list)", "Sum of list"},
			{"mean(list)", "Mean/average of list"},
			{"variance(list)", "Variance of list"},
			{"stddev(list)", "Standard deviation of list"},
			{"median(list)", "Median of list"},
			{"factorial(n)", "n! (factorial)"},
			{"combinations(n, r)", "nCr (combinations)"},
			{"permutations(n, r)", "nPr (permutations)"},
			{"percent(n, total)", "Percentage of n in total"},
		},
	}
}
