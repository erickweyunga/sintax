...
/*
 * Use `addr2line` utility when printing callers to get symbol line numbers.
 * Not supported in case of `malloc` redirection because the current
 * implementation uses `popen()` which may call `malloc()`.
 */
